 ...
 *
 * Signup bonus unified to 20 points.
 *   - A normal (email/phone) signup grants EXACTLY 20 points — no doubled bonus.
 *     (The old, never-registered UserObserver +50 was deleted; the single live
 *      source is RegisteredUserController + config/pricing.registration_welcome_points.)
 *   - A first-time Socialite (Google) signup grants EXACTLY 20 points, matching
 *     the normal signup (no path is left at a higher amount).
 *   - Registration OTP confirmation grants NO extra points (Phase 2 removed the
 *     old +50; the +20 *phone* bonus moved to the profile phone-verification
 *     flow in Phase 5, granted once per lifetime via phone_bonus_claimed_at).
 *   - The earn-guide + footer teaser advertise 20, never 100.
 */
...
it('grants exactly 20 points on a first-time Socialite signup (matches normal signup)', function () {
...
it('grants no extra points on registration OTP confirmation (the +20 moved to the profile phone flow)', function () {
    Queue::fake();

    // Simulate a user who just registered (already holds the 20 welcome points).
    $user = User::factory()->withPendingOtp('5678')->create(['points' => 20]);

    $this->actingAs($user)
        ->post(route('otp.verify'), ['otp' => '5678'])
        ->assertRedirect(route('dashboard'));

    // Registration confirmation grants nothing extra — balance stays 20.
...
it('never credits extra points on repeated registration OTP verification', function () {
    ...
    $user = User::factory()->withPendingOtp('4321')->create(['points' => 20]);
...
it('advertises 20 (not 100) in the earn-guide and footer teaser', function () {
    app()->setLocale('ar');
    ...
    // The register reward row now reads +20; no +100 promise remains on the page.
    expect($html)->toContain('+20')->not->toContain('+100');

    // The footer teaser string itself must read 20, not 100.
    expect(__('ui.footer.gift_teaser'))->toContain('20')->not->toContain('100');
    app()->setLocale('en');
    expect(__('ui.footer.gift_teaser'))->toContain('20')->not->toContain('100');
});
