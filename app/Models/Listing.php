...
class Listing extends Model implements HasMedia
{
    ...
    public function featureWithPoints(int $days): void
    {
        ...
        DB::transaction(function () use ($days, $cost) {
            /** @var User $user */
            $user = User::query()->lockForUpdate()->findOrFail($this->user_id);

            $entitlements = app(EntitlementService::class);

            $isNewFeaturedSlot = ! ($this->is_featured && $this->featured_until?->isFuture());

            if ($isNewFeaturedSlot && ! $entitlements->canUseFeature($user, EntitlementService::FEATURE_FEATURED_LISTINGS_LIMIT, $this)) {
                throw new \Exception(__('server.dashboard.feature_limit_plan'));
            }

            if (! $entitlements->canUseFeature($user, EntitlementService::FEATURE_MONTHLY_BOOST_LIMIT)) {
            ...
            $entitlements->recordUsage($user, EntitlementService::FEATURE_MONTHLY_BOOST_LIMIT);
        });
    }
